"""Aplica consenso 2-way (Claude + Gemini) sobre los 40 posts oficialistas (Jiménez + Cravioto).

Misma lógica que apply_consensus_2way_recent.py pero leyendo los CSVs `_oficialistas`:
- results_claude_oficialistas.csv
- results_gemini_oficialistas.csv

Solo UPDATEa posts donde tono_discurso IS NULL.

Usage:
  docker exec -e DATABASE_URL='postgresql+asyncpg://crece:crece_dev@db:5432/crece' \\
    crece-backend python /app/scripts/apply_consensus_2way_oficialistas.py [--dry-run]
"""
...
CLAUDE_CSV = EVAL_DIR / "results_claude_oficialistas.csv"
GEMINI_CSV = EVAL_DIR / "results_gemini_oficialistas.csv"
CONSENSUS_CSV = EVAL_DIR / "consensus_2way_oficialistas_2026-05-09.csv"

NLP_VERSION = "claude+gemini-2way-oficialistas-2026-05-09"
