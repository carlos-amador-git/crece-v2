...
**Ultimo update:** 2026-07-16/17 · **Actores políticos gobernados (ADR-0009) + BUG-CRECE-1 + retractación SEC-CRECE-1.** Sesión Linda (Fable 5). (1) **SEC-CRECE-1 retractado** con código: admin cross-org es diseño (`dirigentes.py:135-138`); solo quedó BUG-CRECE-1. (2) **BUG-CRECE-1 fix pusheado** (`b303329` en origin): org_id nunca NULL en POST/PATCH + analyst org-scoped (R1 REJECT 2/2 agy+codex → hardening → R2 doble PASS → push con OK CEO). (3) **feat actores (`0ae8829`, LOCAL sin push):** rol_politico obligatorio en alta (422) + migración `5a6505d74dc7` backfill/NOT NULL aplicada dev (0 NULLs: Pepe=independiente decisión CEO, Ivette/Harp=oficialismo) + fallo visible `rol_sin_clasificar` en KPI/UI (mató fallback silencioso `actividad_alineada.py:88`) + wizard captura partido+rol + org sin defaults mágicos (`or 3` muerto) + ADR-0009 Proposed + RUNBOOK-alta-actores. Diseño por tríada `/verifica --diseño` CONSENSO 3/3 (`.context/DISENO-actores-politicos-2026-07-16.md`) + GO CEO. Cross-audit commit: doble PASS (hallazgos aplicados: PATCH null explícito→422, onboard `is not None`). Verificado: pytest 23/23 + onboarding 10 passed (11 errores PREEXISTENTES: fixture `authed_client` no existe en repo) + tsc/lint/no-mocks verdes. (4) **Hannah de Lamadrid (env Carlos):** cliente APARTE (org propia comercial, NO "Morena" a secas), MORENA/oficialismo, display "Hannah", password se queda. (5) **⚠️ Re-clone post rm-projects-wipe:** `.env` raíz PERDIDO (CEO debe restaurar; stack completo no levanta), `.venv`+`node_modules` reconstruidos, hooks gobernanza REINSTALADOS (estaban caídos), volúmenes Docker intactos. (6) Índice ADR desactualizado: `ADR-005-quien-dispara...` y `ADR-006-dedup-fans...` en disco fuera del índice (naming no canónico) — reportado, sin tocar. Gobernanza cross-repo (radar/lexia/obras) inventariada vía peers: los 4 repos L3; portar candidatos = denylist-desde-BD (lexia), gate vigencia documental (radar), branch protection.

---

**2026-06-13 (bloque previo):** **Botón "Sincronizar con RADAR" (ADR-005, Accepted) + 3 dirigentes actualizados.** Feature en `origin/main`: endpoint `/ingest/sync/{id}` (admin/analyst) + UI botón en `/dirigentes/[id]` + badge frescura + skip grácil adapters. Construido por `gemini --yolo` (lanzado por Linda; error de yolo reconocido), verificado/consolidado por Linda (ejecutor≠auditor). **Verificado E2E EN LA APP** (Playwright): botón visible+funcional (Felipe/Pepe), camino "ingiere data nueva" probado (Pepe +51 posts vía sync, Saymi +85). **3 dirigentes al día (medido):** Saymi followers 287,097 (FB 244k) post 06-13 · Pepe 27,875 post 06-11 · Felipe 7,243 post 06-13. **2 bugs cazados verificando (no confiando en COMPLETED):** flush en `discover_manifest` (botón daba 500) + adapter followers no leía wrapper RADAR (128k followers FB de Saymi ocultos). Tests 16+4 verdes. **Pendientes:** ~~IG-posts Saymi~~ RESUELTO (214 IG-posts, 30 nuevos del 06-13 ya ingeridos vía botón; credenciales chatmx_oficial = futuras corridas RADAR) · deploy Coolify NO urgente (sin data/bundles ahí + necesita env MINIO_*) · pipeline auto RADAR→CRECE (n8n workflow inactive) · NLP fine-tune (faltan 2 de 3 Excels evaluadores). **3er bug cazado (tarde) — fans inflados ~2x:** el ranking de fans sumaba perfiles duplicados del mismo fan (RADAR cambia `profile_external_id` entre capturas → N watched_profiles, Saymi 95k perfiles vs 73k reales). Pedro Carlock mostraba 988 (=suma de 4 perfiles) vs 514 real. **Fix `8fe548c` (ADR-006):** dedup por `author_hash` en ranking + conteo (`COUNT(DISTINCT)`), NO toca BD. Verificado: Pedro 514 dedupeado; Misael sigue Fan #1 (splice, ADR-0002/0005/0007). NO afecta D's/IPD/bots (no usan watched_profiles). **Deudas escaladas (ADR-006):** causa raíz RADAR (UPSERT por author_hash, sino crece 23%→50%/mes) · normalización nombres (18k variantes) · recalibrar Misael >514 (regla 12, requiere PR CEO).
**Git:** `origin/main @ 342a52f`, sincronizado, cero pendientes. data/PII en `.gitignore`.
Lección de la sesión: usar la app + verificar dato real + leer-antes-de-construir (casi dupliqué 3×) + no atribuir sin fundamento (culpé a "Antigravity" sin verificar que era mi propio gemini-yolo).

---

**2026-05-27 · CRASH RAM (máquina al borde + factores concurrentes) → reboot. **NADA se perdió.** ⚠️ CORRECCIÓN de un error mío previo: dije "data 3 MC perdida" — FALSO, fue join a tabla equivocada (`watched_profiles`=audiencia/likers en vez de `social_profiles`=perfil propio del dirigente, que es a lo que apunta `social_posts.profile_id`). Join correcto: **data INTACTA en crece-db :5438** — Piña(1) 522 posts/494 tono/2922 comments · Solano(2) 353/234/147 · Ballesteros(8) 685/271/41868. Enrich iba 40-95% al tronar. **Trabajo real (chico, NO re-ingest):** (1) parchear 4 scripts NLP con `--strict-mcp-config` (validado: apaga MCP sin romper auth; `--bare` NO sirve, exige API key); (2) correr `post_ingest_enrich.py --dirigente-id {1,8,2}` attended/lotes/idempotente (solo delta sin enriquecer), CC_MODEL=sonnet medium; (3) verificar app. Enrich target :5438 (NO :5453=radar scraper crudo).
