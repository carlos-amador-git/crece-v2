...
- **Snapshot de followers NO actualiza el dashboard solo:** ingestar `social_profile_snapshots` (B07) NO propaga a `social_profiles.followers_count`. El dashboard "Tu Audiencia / Seguidores" suma `SocialProfile.followers_count` (`dashboard.py:212`), NO el snapshot. Tras ingestar snapshots, propagar: `UPDATE social_profiles sp SET followers_count=s.fc FROM (SELECT DISTINCT ON(profile_id) profile_id, followers_count fc FROM social_profile_snapshots WHERE followers_count>0 ORDER BY profile_id, taken_at DESC) s WHERE sp.id=s.profile_id`. (Dirigente nuevo nace con followers_count=0 → "Tu Audiencia: 0" hasta propagar.)
